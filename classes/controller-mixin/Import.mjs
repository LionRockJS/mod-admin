...
export default class ControllerMixinImport extends ControllerMixin {
    ...
    static init(state) {
        ...
        if (!state.get(this.UNIQUE_KEY))
            state.set(this.UNIQUE_KEY, 'email');
        if (!state.get(this.HEADER_COLUMN_MAP))
            state.set(this.HEADER_COLUMN_MAP, new Map());
        if (!state.get(this.ENCODING))
            state.set(this.ENCODING, 'utf8');
        if (!state.get(this.PARSER_OPTIONS))
            state.set(this.PARSER_OPTIONS, {
                delimiter: ',',
                skip_empty_lines: true,
                skip_records_with_error: true
            });
        if (!state.get(this.IMPORT_CSV_HANDLER))
            state.set(this.IMPORT_CSV_HANDLER, async (state, csvHeader, csvRecords) => { });
        if (!state.get(this.IMPORT_INSTANCE_HANDLER))
            state.set(this.IMPORT_INSTANCE_HANDLER, async (state, instance, csvRecord) => { });
    }
    static async readCSV(state) {
        ...
        if (records.length <= 1)
            throw new Error('no record in csv file');
    ...
    static async action_import_post(state) {
        ...
        if (uniqueKeyIndex === -1)
            throw new Error('unique key not found in header map');
        ...
        await Promise.all(records.map(async (values) => {
