...
import {ControllerMixinView, ControllerMixin, Controller, ControllerState, View} from '@lionrockjs/central';
...
export default class ControllerMixinExport extends ControllerMixin{
  ...
  static init(state: Map<string, any>) {
    ...
    if (!state.get(this.EAGER_LOAD_FUNCTION)) state.set(this.EAGER_LOAD_FUNCTION, async (state: any) =>{/***/});
    if (!state.get(this.EXPORT_INSTANCE_HANDLER)) state.set(this.EXPORT_INSTANCE_HANDLER, async (state: any, instance: any) => {/***/});
    if (!state.get(this.EXPORT_INSTANCES_FILTER)) state.set(this.EXPORT_INSTANCES_FILTER, async (state: any) => {/***/});
  }

  static async action_export(state: Map<string, any>){
    // set response header
    ...
    filteredInstances.forEach((it: any) =>{
      if(!it) return;
    ...
    // Add CSV header
    rows.unshift([...columns.values()].map((x: any)=> (typeof x === 'string') ? `"=""${x}"""`: x).join(','))
