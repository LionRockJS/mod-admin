...
export default class ControllerMixinImport extends ControllerMixin {
  ...
  static init(state: Map<string, any>) {
    ...
    if(!state.get(this.IMPORT_CSV_HANDLER)) state.set(this.IMPORT_CSV_HANDLER, async (state: any, csvHeader: any, csvRecords: any) => {/***/});
    if(!state.get(this.IMPORT_INSTANCE_HANDLER)) state.set(this.IMPORT_INSTANCE_HANDLER, async (state: any, instance: any, csvRecord: any) => {/***/});
  }

  static async readCSV(state: Map<string, any>){
  ...
  static async writeRecord(state: Map<string, any>, instance: any, csvRecord: any){
  ...
  static async action_import(state: Map<string, any>) {/***/}

  static async action_import_post(state: Map<string, any>) {
    ...
    if(headerColumnMap.size === 0){
      [...Model.fields.keys()].forEach((it: any)=>headerColumnMap.set(it,it));
    }

    //verify csv
    //get index of unique key from headers
    const uniqueKeyIndex = csvHeader.findIndex((it: any) => headerColumnMap.get(it) === importUniqueKey);
    if(uniqueKeyIndex === -1) throw new Error('unique key not found in header map');

    const columns = csvHeader.map((it: any) => {
    ...
    await Promise.all(
      records.map(async (values: any) => {
        //map csv value to database column
        const result: any = {};
        values.forEach((value: any, j: any) => {
