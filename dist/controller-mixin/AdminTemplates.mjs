...
export default class ControllerMixinAdminTemplates extends ControllerMixin {
    ...
    static init(state) {
        if (!state.get(this.PATH_PREFIX))
            state.set(this.PATH_PREFIX, 'admin/');
        if (!state.get(this.ADMIN_DATABASE_KEY))
            state.set(this.ADMIN_DATABASE_KEY, 'admin');
        if (!state.get(this.PAGE_SIZE))
            state.set(this.PAGE_SIZE, 50);
        if (!state.get(this.TEMPLATES))
            state.set(this.TEMPLATES, new Map([
        ...
        if (!state.get(this.DEFAULT_TEMPLATES))
            state.set(this.DEFAULT_TEMPLATES, new Map([
                ['index', 'templates/admin/index'],
                ['read', 'templates/admin/edit'],
                ['edit', 'templates/admin/edit'],
                ['create', 'templates/admin/edit'],
    ...
    static async entitySupport(state) {
        ...
        if (!entity)
            return (checkpoint) ? { destination: checkpoint } : {};
    ...
    static async modelToTemplateData(state) {
        ...
        try {
            return Object.assign(this.getFieldData(instance), await this.getBelongsTo(state, instance), await this.getBelongsToMany(state, instance), await this.getHasMany(state, instance), this.getFormDestination(state), this.getDomain(state));
        }
        catch (e) {
    ...
    static async getBelongsTo(state, instance) {
        const m = this.classObject(instance.constructor);
        if (!m.belongsTo || m.belongsTo.length <= 0)
            return {};
        const items = await Promise.all([...m.belongsTo].map(async (x) => {
            const fk = x[0];
    ...
    static async getBelongsToMany(state, instance) {
        const m = this.classObject(instance.constructor);
        if (!m.belongsToMany || m.belongsToMany.length <= 0)
            return {};
        const items = await Promise.all(Array.from(m.belongsToMany).map(async (x) => {
    ...
    static async getHasMany(state, instance) {
        ...
        if (!m.hasMany || m.hasMany.length <= 0)
            return {};
        const request = state.get(ControllerState.REQUEST);
        const { id } = state.get(ControllerState.PARAMS);
        const items = await Promise.all(m.hasMany.map(async (x) => {
            const fk = x[0];
            const Model = await ORM.import(x[1]);
