...
export default class ControllerMixinExport extends ControllerMixin {
    ...
    static init(state) {
        if (!state.get(this.COLUMNS))
            state.set(this.COLUMNS, new Map([['created_at', 'Date']]));
        if (!state.get(this.EAGER_LOAD_FUNCTION))
            state.set(this.EAGER_LOAD_FUNCTION, async (state) => { });
        if (!state.get(this.EXPORT_INSTANCE_HANDLER))
            state.set(this.EXPORT_INSTANCE_HANDLER, async (state, instance) => { });
        if (!state.get(this.EXPORT_INSTANCES_FILTER))
            state.set(this.EXPORT_INSTANCES_FILTER, async (state) => { });
    }
