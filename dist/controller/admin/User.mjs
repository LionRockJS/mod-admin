...
export default class ControllerAdminUser extends ControllerAdmin {
    ...
    async action_index() {
        ...
        await Promise.all(instances.map((user) => user.eagerLoad({ with: ['Person'] })));
        instances.forEach((user) => user.name = user.person.first_name + (user.person.last_name ? ` ${user.person.last_name}` : ""));
    }
    async action_read() {
